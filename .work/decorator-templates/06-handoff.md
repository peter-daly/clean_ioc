...
Status: Implementation verified; coordinator checkpoint and independent Astra High review pending.
...
- Generated/ordinary sorting retains position, layer and reverse declaration ordinal. For ordinal collisions caused
  by inherited patches, stable declaration traversal rank precedes reverse source ordinal, keeping each template's
  source layers together. Ordinary definitions precede generated definitions in a traversal tie; templates follow
  retained layer tuple order. Activation runs inside-out, so the later tied declaration is outermost. This follows the
  established stable ordinary-layer convention without modifying stored ordinals or inventing UUID-based ordering.
...
Changed production file: `clean_ioc/container.py`. New `tests/test_decorator_template_composition.py`: **27 cases**.
...
**573 passed in 4.72s.** New composition file alone: **27 passed in 0.52s**.
...
**350 passed, 4 skipped in 2.49s.** All 27 new cases pass; skips are the four previously documented Python-version cases
...
Independent review has not run and no acceptance is claimed. Coordinator must checkpoint verification, then obtain the
separate prescribed Astra High review. Detailed diagnostics/inspection remain M07; bark-core integration M08; public
