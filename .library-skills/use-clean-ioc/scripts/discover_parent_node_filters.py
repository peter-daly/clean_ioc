#!/usr/bin/env python3
"""Discover parent-node filters exposed by the installed Clean IoC version."""
...
def main() -> int:
    ...
    for name in node_filters.__all__:
        node_filter = getattr(node_filters, name)
        signature = str(inspect.signature(node_filter))
        docstring = inspect.getdoc(node_filter) or ""
        if _matches_search(name, signature, docstring, args.terms):
            matches.append((name, signature, docstring))

    if not matches:
        print("No parent-node filters matched the search terms.")
        return 1
